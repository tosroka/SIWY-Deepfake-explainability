...
columns = [
    description[0] for description in conn.execute(sql_query).description
]
...
def download_full_song(row, out_dir="data/lastfm/audio"):
    ...
    if os.path.join(out_dir, f"{row[0]}.mp3") in glob.glob(
        os.path.join(out_dir, "*.mp3")
    ):
        return None
    ...
    try:
        ...
        command = [
            ...
            "mp3",  # Convert to mp3
            "--output",
            f"{out_dir}/{row[0]}.%(ext)s",  # Output filename
            youtube_url,
        ]
