...
from training.solver import skip_files
...
class Predict(object):
    ...
    def get_dataset(self):
        ...
        if self.dataset == "msd":
            ...
            self.test_list = [
                ...
                if value.decode() not in skip_files
            ]
            id2tag_file = os.path.join(
                "./../split/msd", "msd_id_to_tag_vector.cP"
            )
            self.id2tag = pickle.load(
                open(id2tag_file, "rb"), encoding="bytes"
            )
    ...
    def get_tensor(self, fn):
        ...
        if self.dataset == "mtat":
            npy_path = (
                os.path.join(
                    self.data_path, "mtat", "npy", fn.split("/")[1][:-3]
                )
                + "npy"
            )
        ...
        for i in range(self.batch_size):
            x[i] = torch.Tensor(
                raw[i * hop : i * hop + self.input_length]
            ).unsqueeze(0)
        return x

    def get_auc(self, est_array, gt_array):
        roc_aucs = metrics.roc_auc_score(gt_array, est_array, average="macro")
        pr_aucs = metrics.average_precision_score(
