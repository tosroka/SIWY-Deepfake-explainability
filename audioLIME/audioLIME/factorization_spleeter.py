...
class SpleeterFactorization(SourceSeparationBasedFactorization):
    ...
    def __init__(
        ...
        super().__init__(
            input, target_sr, temporal_segmentation_params, composition_fn
        )
    ...
    def initialize_components(self):
        ...
        waveform = librosa.resample(
            waveform, orig_sr=self.target_sr, target_sr=spleeter_sr
        )
...
class SpleeterPrecomputedFactorization(SpleeterFactorization):
    def __init__(
        ...
        target_sr=16000,
        model_name="spleeter:5stems",
        spleeter_sources_path=None,
        recompute=False,
    ):
        assert isinstance(
            input, str
        ), "input must be file path. otherwise use SpleeterFactorization."
        if spleeter_sources_path is None:
            raise TypeError(
                "spleeter_sources_path must not be None. "
                "Provide path or use SpleeterFactorization."
            )
        self.spleeter_sources_path = os.path.join(
            spleeter_sources_path, model_name
        )
    ...
    def initialize_components(self):
        ...
        precomputed_path = os.path.join(
            self.spleeter_sources_path, precomputed_name
        )
        if self.recompute:
            waveform = self._original_mix
            separator = Separator(self.model_name, multiprocess=False)
...
if __name__ == "__main__":
    ...
    for i, audio in enumerate(audio_list):
        ...
        factorization = SpleeterPrecomputedFactorization(
            input,
