...
class Augmenter:
    ...
    def stft(self, x, mode = 'complex'):
        ...
        print("doing stft with params -xshape", x.shape, "fft params", self.params['fft'])
        ...
        if mode == 'magnitude':
            ...
            print("shape before return", complex_x.shape)
            ret = tf.expand_dims( tf.math.log( tf.clip_by_value(
                        tf.square(tf.abs(complex_x)),
                        1e-10,
                        1e6, )
                    ) / tf.math.log( tf.constant(10., dtype=tf.float32)), -1)
            print("after return", ret.shape)
            return ret
