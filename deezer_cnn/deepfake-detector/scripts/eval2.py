...
configuration.load_model(args.config) # loads model specific config on top of base 
...
if not ENCODER:
    it_test = it_test.map(lambda x, y: (x, one_hot_encoder(y, 10)) ) # loader.n_encoders+1 #missing data is padded!
else:
...
loader.params['patch_size_f'] = 64
loader.params['patch_size_f_min'] = 64 # Required by patch_batch_spec logic
...
n_encoders = 10 #loader.n_encoders+1
if ENCODER:
...
for encoder in loader.encoders:
    ...
    if not ENCODER:
        fake_it = fake_it.map(lambda x, y: (x, one_hot_encoder(y, 10)) )
    else:
...
real_it = real_it.map(lambda x, y: (x, one_hot_encoder(y, 10)) ) # loader.n_encoders+1
if 'use_raw' in global_conf:
